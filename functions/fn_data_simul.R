fn_data_simul = function(ncurve, Sig = c(1, .5)^2, sig = .05, #seedn = 1234,
                         nbasis = 10, norder = 5, ntime = 1000, alpha){
  ...
  ytobs = hobs + sigma * rt(length(hobs), df = 4, ncp = 0)
  dat.sim = data.frame(id, tobs, wobs, hobs, yobs, ytobs)
  
  return(list(dat = dat.sim, par = true.par))
}


fn_data_tsimul = function(ncurve, Sig = c(1, .5)^2, sig = .05, #seedn = 1234,
                         nbasis = 10, norder = 5, ntime = 1000, alpha){
  ...
  yobs = hobs + sigma * rt(length(hobs), df = 4, ncp = 0)
